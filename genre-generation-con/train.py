...
def main():
	...
	parser.add_argument('--log_dir', type=str, default="/data/anussank/shavak/GAN/genre_con_lm/",
	...
	with tf.variable_scope(tf.get_variable_scope()):
		...
		with tf.variable_scope('embeddings_encoder') as scope:
			...
			genre_embedding = tf.get_variable('genre_embeddings',
			                            initializer=tf.truncated_normal([len(loaded_data.keys()), 100], mean=0.0,
			                                                            stddev=0.01), trainable=True)
		...
		g_optim = tf.train.AdamOptimizer(args.learning_rate, beta1 = args.beta1).minimize(-1.0 * loss['g_loss'], var_list=variables['g_vars'])
		...
		with tf.variable_scope(tf.get_variable_scope(), reuse=True) as scope:
			...
			ass_6 = tf.get_variable("g_rnn/output_project/Matrix").assign(np.load(
				"/data/anussank/shavak/GAN/indiv_GRU_OOV/Model_softmax_w:0.npy"))
			ass_7 = tf.get_variable("g_rnn/output_project/bias").assign(np.load(
				"/data/anussank/shavak/GAN/indiv_GRU_OOV/Model_softmax_b:0.npy"))
		...
		with sv.managed_session(config=protoconfig) as sess:
			...
			sess.run([ass_1, ass_2, ass_3, ass_4, ass_5, ass_6, ass_7])
