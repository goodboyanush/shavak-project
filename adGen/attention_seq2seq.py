...
def attention_decoder(decoder_inputs, initial_state, attention_states, cell,
  ...
  with variable_scope.variable_scope(scope or "attention_decoder"):
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      x = rnn_cell._linear([inp] + attns, cell.state_size, True)
      # Run the RNN.
...
def embedding_attention_decoder(decoder_inputs, initial_state, attention_states,
  ...
  with variable_scope.variable_scope(embedding_scope or "embedding_attention_decoder",reuse = True) as s1:
    
    print('Insider variable scope for embedding in decoder ' + embedding_scope.name)
    embedding = tf.get_variable("embeddings")
    print('Insider variable scope for embedding in decoder ' + embedding.name)
  ...
  Returns:
    A tuple of the form (outputs, state), where:
      ...
      state: The state of each decoder cell at the final time-step.
        It is a 2D Tensor of shape [batch_size x cell.state_size].
  """
  with variable_scope.variable_scope(scope or "embedding_attention_seq2seq", dtype=dtype, reuse=True) as scope:

    embedding_scope = scope
    embeddings = tf.get_variable("embeddings")
    emb_encoder_inputs = [embedding_ops.embedding_lookup(embeddings, x) for x in encoder_inputs]
  
  with variable_scope.variable_scope("embedding_attention_seq2seq", dtype=dtype) as scope:
    # Encoder.
    encoder_cell = cell
    encoder_outputs, encoder_state = rnn.rnn(encoder_cell, emb_encoder_inputs, dtype=dtype)

    # encoder_outputs, encoder_state = emb_encoder_inputs
    ...
    if isinstance(feed_previous, bool):
      return embedding_attention_decoder(
          ...
          num_decoder_symbols,embedding_size=embedding_size, num_heads=num_heads, output_size=output_size,
          output_projection=output_projection, feed_previous=feed_previous,
          initial_state_attention=initial_state_attention, embedding_scope=embedding_scope)
