...
def selu(x):
	with tf.variable_scope('selu') as scope:
...
class GAN:
	...
	def build_model(self):
		...
		t_not_genre_sent = tf.placeholder('int64', [self.options['batch_size'], self.options['seq_len']],
										  name='wrong_genre')
		t_genre = tf.placeholder('int64', [self.options['batch_size']], name='genre')
		t_z = tf.placeholder('float32', [self.options['batch_size'], self.options['z_dim']])
		# t_labels = tf.placeholder('float32', [self.options['batch_size'], 5])
		zeros = tf.random_normal([self.options['batch_size'], 1], 0.15, 0.15)
		ones = tf.random_normal([self.options['batch_size'], 1], 0.95, 0.25)
		...
		generated_sentence = self.generator(t_z, genre_list, embeddings)
		# GENRE
		...
		d_loss1 = tf.reduce_mean(
			tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_genre_sent_logits, labels=ones))
		d_loss2 = tf.reduce_mean(
			tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_not_genre_logits, labels=zeros))
		d_loss3 = tf.reduce_mean(
			tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_gen_sent_logits, labels=zeros))
		...
		d_vars.append(embeddings)
		d_vars.append(genre_embeddings)
		g_vars = [var for var in t_vars if 'g_' in var.name]
		g_vars.append(embeddings)
		g_vars.append(genre_embeddings)
		input_tensors = {
	...
	def sampler(self, t_z, t_text_embedding):
		...
		output, state = ops.rnn(z_, seq_len=self.options['seq_len'], cell=cell, name='g_rnn',
								batch_size=self.options['batch_size'], vocab_size=self.options['vocab_size'], embeddings=embeddings, inference=True)
		print(output)
		output = tf.expand_dims(output, axis=-1)
		return output

	# GENERATOR IMPLEMENTATION based on : https://github.com/carpedm20/DCGAN-tensorflow/blob/master/model.py
	def generator(self, t_z, t_text_embedding, embeddings):
		...
		cell = tf.contrib.rnn.GRUCell(num_units=self.options['num_lstm_units'])
		output, state = ops.rnn(z_, seq_len=self.options['seq_len'], cell=cell, name='g_rnn',
								batch_size=self.options['batch_size'], vocab_size=self.options['vocab_size'],
